...
void main()
{
	...
	vec4 lightmap_color = texture2D(lightmap, lm);

	lightmap_color.rgb = (0.5 * lightmap_color.rgb) + 0.5;

	color *= lightmap_color;

	float lightmap_luminance = (lightmap_color.r + lightmap_color.g + lightmap_color.b) / 3.0;

	color.rgb = mix(color.rgb + ( 0.5 * smoothstep(0.0, 1.0, lightmap_color.rgb - 0.5) ), color.rgb, 1.0-lightmap_luminance);
